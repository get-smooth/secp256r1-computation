...
type ProjectivePoint = ReturnType<typeof secp256r1.ProjectivePoint.fromPrivateKey>;
type AffineProjectivePoint = ReturnType<typeof secp256r1.ProjectivePoint.fromAffine>;

const fuzzer = () => {
    const NUMBER_OF_TESTS = 20;
    const MAX_POINTS_IN_ARRAY = 15;
    // generate a list of random public keys before running the tests
    const PARAMS_RANDOM_PUBLIC_KEYS: ProjectivePoint[] = new Array(NUMBER_OF_TESTS).fill(0).map(generateRandomPubKey);
    // generate a list of a list of random points before running the tests
    const PARAMS_RANDOM_POINTS: AffineProjectivePoint[][] = new Array(NUMBER_OF_TESTS).fill(0).map(() => {
    ...
    /*//////////////////////////////////////////////////////////////
                                 TESTS
    //////////////////////////////////////////////////////////////*/
    it.each(PARAMS_RANDOM_PUBLIC_KEYS)('concatenates the x and y coordinates of a single point', (pubKey) => {
        const points = [{ ...pubKey, x: pubKey.px, y: pubKey.py }] satisfies AffineProjectivePoint[];

        const result = concatenatePoints(points);
        expect(result).toEqual(`${leftPadCoord(pubKey.px)}${leftPadCoord(pubKey.py)}`);
    });

    it.each([PARAMS_RANDOM_POINTS])('concatenates the x and y coordinates of multiple points ', (points) => {
        const result = concatenatePoints(points);
